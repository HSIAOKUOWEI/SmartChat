...
from llm_config.groq_model import groq_llm
...
llm = groq_llm(groq_api_key=groq_api_key,model_name="llama3-groq-70b-8192-tool-use-preview")

agent = create_tool_calling_agent(llm=llm, tools=tools, prompt=prompt)
agent_executor = AgentExecutor(agent=agent, tools=tools) # ,verbose=True
# print(agent_executor.invoke({"input": "宜蘭和金門的天氣如何"}))

for chunk in agent_executor.stream({"input": "寫一個排序算法"}):
    if "actions" in chunk:
